...
class Database(object):
    ...
    MAX_WRITES_PER_TRANSACTION = 2000_000
    ...
    def sql(
        ...
        try:
            ...
            if debug:
                time_end = time()
                frappe.errprint(
                    ("Execution time: {0} sec").format(round(time_end - time_start, 2))
                )
    ...
    def get_single_value(self, doctype, fieldname, cache=True):
        ...
        if not df:
            frappe.throw(
                _("Invalid field name: {0}").format(frappe.bold(fieldname)),
    ...
    def set_value(
        ...
        **Warning:** this function will not call Document events and should be avoided in normal cases.
        ...
        if is_single_doctype:
            frappe.db.delete(
                "Singles",
                filters={"field": ("in", tuple(to_update)), "doctype": dt},
                debug=debug,
            )
            ...
            if for_update:
                docnames = tuple(
                    self.get_values(
                        dt, dn, "name", debug=debug, for_update=for_update, pluck=True
                    )
                ) or (NullValue(),)

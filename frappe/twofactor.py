...
def two_factor_is_enabled(user=None):
    ...
    if enabled:
        bypass_two_factor_auth = int(
            frappe.db.get_value(
                "System Settings", None, "bypass_2fa_for_retricted_ip_users"
            )
            or 0
        )
...
def get_link_for_qrcode(user, totp_uri):
    ...
    lifespan = (
        int(
            frappe.db.get_value(
                "System Settings", "System Settings", "lifespan_qrcode_image"
            )
        )
        or 240
    )
...
def reset_otp_secret(user):
    ...
    if frappe.session.user in ["Administrator", user]:
        ...
        email_args = {
            "recipients": user_email,
            "sender": None,
            "subject": _("OTP Secret Reset - {0}").format(
                otp_issuer or "Frappe Framework"
            ),
            "message": _(
